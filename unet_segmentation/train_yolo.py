...
Dataset: Format YOLO (images/ & labels/ yang terbagi atas train/val/test).
"""
...
# Import class YOLODataset yang baru dari modul dataset Anda
...
def main() -> int:
    ap = argparse.ArgumentParser(description="U-Net segmentasi (YOLO format dataset)")
    root = Path(__file__).resolve().parent.parent
    ap.add_argument(
        "--yolo-root",
        type=Path,
        default=root / "yolo_dataset",
        help="Root folder dataset format YOLO (berisi images/ dan labels/)",
    )
    ...
    ap.add_argument("--encoder", type=str, default="resnet34", help="Encoder smp, mis. resnet34")
    ...
    if not yolo_root.is_dir():
        print(f"yolo_root dataset tidak ditemukan: {yolo_root}", file=sys.stderr)
        return 1
    ...
    # --- LOADING DATASET VIA YOLODATASET ---
    try:
        ...
        print(f"Gagal memuat dataset format YOLO: {e}", file=sys.stderr)
        return 1

    if len(train_ds) == 0 or len(val_ds) == 0:
        print(f"Data kosong! Train: {len(train_ds)}, Val: {len(val_ds)}", file=sys.stderr)
        return 1

    print(f"Berhasil memuat data format YOLO. Sampel train: {len(train_ds)}, val: {len(val_ds)}")
    ...
    box_loss = nn.BCEWithLogitsLoss()  # Digunakan bersama bce_loss untuk kestabilan gradien
